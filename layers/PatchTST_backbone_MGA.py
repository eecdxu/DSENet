...
class PatchTST_backbone(nn.Module):
    ...
    def forward(self, z):                                                                   # z: [bs x nvars x seq_len]
        ...
        # model
        z_backbone, atten3 = self.backbone(z)                                                            # z: [bs x nvars x d_model x patch_num]
        atten1, atten2 = None, None
...
class TSTiEncoder(nn.Module):  #i means channel-independent
    ...
    def forward(self, x) -> Tensor:                                              # x: [bs x nvars x patch_len x patch_num]
        ...
        # Encoder
        z, attn3 = self.encoder(u)                                                      # z: [bs * nvars x patch_num x d_model]
...
class TSTEncoder(nn.Module):
    def __init__(self, q_len, d_model, n_heads, d_k=None, d_v=None, d_ff=None, 
                        ...
                        res_attention=False, n_layers=1, pre_norm=False, store_attn=False,pe='zeros', learn_pe=True):
        super().__init__()
    ...
    def forward(self, src:Tensor, key_padding_mask:Optional[Tensor]=None, attn_mask:Optional[Tensor]=None):
        ...
        if self.res_attention:
            for mod in self.layers: output, attn, scores = mod(output, prev=scores, key_padding_mask=key_padding_mask, attn_mask=attn_mask)
            return output, attn, scores
        else:
            for mod in self.layers: output, attn = mod(output, key_padding_mask=key_padding_mask, attn_mask=attn_mask)
            return output, attn
...
class TSTEncoderLayer(nn.Module):
    def __init__(self, q_len, d_model, n_heads, d_k=None, d_v=None, d_ff=256, store_attn=False,
        ...
        self.res_attention = res_attention
        self.self_attn = _MultiheadAttention(q_len, d_model, n_heads, d_k, d_v, attn_dropout=attn_dropout, proj_dropout=dropout, res_attention=res_attention,
                                             act=activation, pe=pe, learn_pe=learn_pe)
