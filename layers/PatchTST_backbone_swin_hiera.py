...
class PatchTST_backbone(nn.Module):
    def __init__(self, c_in:int, context_window:int, target_window:int, patch_len:int, stride:int, max_seq_len:Optional[int]=1024, 
        ...
        # Head
        self.head_nf = self.d_model * patch_num//4
        self.n_vars = c_in
...
class TSTiEncoder(nn.Module):  #i means channel-independent
    ...
    def forward(self, x) -> Tensor:                                              # x: [bs x nvars x patch_len x patch_num]
        ...
        if self.res_attention:
            ...
            z = torch.reshape(z, (B, n_vars, self.patch_num//4, -1))                # z: [bs x nvars x patch_num x d_model]
            z = z.permute(0,1,3,2)                                                   # z: [bs x nvars x d_model x patch_num]
            return z, atten1, atten2, atten3 
...
class TSTEncoder1(nn.Module):
    def __init__(self, pe, learn_pe, patch_num, q_len, num_words, inner_dim, d_model, n_heads, d_k=None, d_v=None, d_ff=None, norm='BatchNorm', attn_dropout=0., dropout=0.,embedding_dropout=0.1,
        ...
        self.res_attention = res_attention
        self.patch_merging = PatchMerging(dim=160)
...
class TSTEncoder2(nn.Module):
    def __init__(self, pe, learn_pe, patch_num, q_len, num_words, inner_dim, d_model, n_heads, d_k=None, d_v=None, d_ff=None, norm='BatchNorm', attn_dropout=0., dropout=0.,embedding_dropout=0.1,
        ...
        self.layers = nn.ModuleList([TSTEncoderLayer2(pe, learn_pe, patch_num, q_len//2, num_words, inner_dim, d_model, n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
                                                      attn_dropout=attn_dropout, dropout=dropout,embedding_dropout=embedding_dropout,
                                                      activation=activation, res_attention=res_attention,
                                                      pre_norm=pre_norm, store_attn=store_attn) for i in range(n_layers)])
        self.res_attention = res_attention
...
class TSTEncoder3(nn.Module):
    def __init__(self, pe, learn_pe, patch_num, q_len, num_words, inner_dim, d_model, n_heads, d_k=None, d_v=None, d_ff=None, norm='BatchNorm', attn_dropout=0., dropout=0.,embedding_dropout=0.1,
        ...
        self.layers = nn.ModuleList([TSTEncoderLayer2(pe, learn_pe, patch_num, q_len//4, num_words, inner_dim, d_model, n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
                                                      attn_dropout=attn_dropout, dropout=dropout,embedding_dropout=embedding_dropout,
                                                      activation=activation, res_attention=res_attention,
                                                      pre_norm=pre_norm, store_attn=store_attn) for i in range(n_layers)])
        self.res_attention = res_attention
...
class TSTEncoderLayer(nn.Module):
    ...
    def forward(self, src:Tensor, B, n_vars, prev1:Optional[Tensor]=None, prev2:Optional[Tensor]=None, key_padding_mask:Optional[Tensor]=None, attn_mask:Optional[Tensor]=None) -> Tensor:
        ...
        if self.res_attention:
            return outer, attn2, scores2
        else:
            return outer, attn2

class TSTEncoderLayer2(nn.Module):
    def __init__(self, pe, learn_pe, patch_num, q_len, num_words, inner_dim, d_model, n_heads, d_k=None, d_v=None, d_ff=256, store_attn=False,norm='BatchNorm', attn_dropout=0, dropout=0.,embedding_dropout=0.1,
                 bias=True, activation="gelu", res_attention=False, pre_norm=False):
        super().__init__()
        # print(num_words, d_model)
    ...
    def forward(self, src:Tensor, B, n_vars, prev1:Optional[Tensor]=None, prev2:Optional[Tensor]=None, key_padding_mask:Optional[Tensor]=None, attn_mask:Optional[Tensor]=None) -> Tensor:
        ...
        ## Multi-Head attention
        if self.res_attention:
            src2_outer, attn2, scores2 = self.self_attn2(src, src, src, prev2, key_padding_mask=key_padding_mask, attn_mask=attn_mask)
        else:
            src2_outer, attn2 = self.self_attn2(src, src, src, key_padding_mask=key_padding_mask, attn_mask=attn_mask)
        if self.store_attn:
            self.attn = attn2
        ## Add & Norm
        outer = src + self.dropout_attn(src2_outer) # Add: residual connection with residual dropout
        if not self.pre_norm:
            outer = self.norm_attn(outer)
