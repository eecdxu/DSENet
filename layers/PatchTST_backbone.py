...
class TSTiEncoder(nn.Module):  #i means channel-independent
    def __init__(self, c_in, patch_num, patch_len, max_seq_len=1024,
        ...
        # Encoder
        self.encoder = TSTEncoder(pe, learn_pe, patch_num, q_len, self.num_words, inner_dim, d_model, n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm, attn_dropout=attn_dropout, dropout=dropout,embedding_dropout=embedding_dropout,
    ...
    def forward(self, x) -> Tensor:                                              # x: [bs x nvars x patch_len x patch_num]
        ...
        # Encoder
        z = self.encoder(inner_tokens, B, n_vars)                                                      # z: [bs * nvars x patch_num x d_model]
...
class TSTEncoder(nn.Module):
    ...
    def forward(self, src:Tensor, B, n_vars, key_padding_mask:Optional[Tensor]=None, attn_mask:Optional[Tensor]=None):
        ...
        if self.res_attention:
            for mod in self.layers: output, scores1, scores2 = mod(output, B, n_vars, prev1=scores1, prev2=scores2, key_padding_mask=key_padding_mask, attn_mask=attn_mask)
            return output
        else:
            for mod in self.layers: output = mod(output, B, n_vars, key_padding_mask=key_padding_mask, attn_mask=attn_mask)
            return output
...
class TSTEncoderLayer(nn.Module):
    ...
    def forward(self, src:Tensor, B, n_vars, prev1:Optional[Tensor]=None, prev2:Optional[Tensor]=None, key_padding_mask:Optional[Tensor]=None, attn_mask:Optional[Tensor]=None) -> Tensor:
        ...
        ## Multi-Head attention
        if self.res_attention:
            src2, attn, scores1 = self.self_attn1(src, src, src, prev1, key_padding_mask=key_padding_mask, attn_mask=attn_mask)
            ...
            src2_outer, attn, scores2 = self.self_attn2(outer, outer, outer, prev2, key_padding_mask=key_padding_mask, attn_mask=attn_mask)
        else:
            src2, attn = self.self_attn1(src, src, src, key_padding_mask=key_padding_mask, attn_mask=attn_mask)
            # outer = outer + self.proj_norm2(self.proj(self.proj_norm1(src2.reshape(B*n_vars, self.patch_num, -1)))) # B, N, C
            outer = outer + self.proj1(src2.reshape(B*n_vars, self.patch_num, -1)) # B, N, C
            src2_outer, attn = self.self_attn2(outer, outer, outer, key_padding_mask=key_padding_mask, attn_mask=attn_mask)
        if self.store_attn:
            self.attn = attn
        ## Add & Norm
        ...
        if self.res_attention:
            return outer.reshape(B*n_vars*self.patch_num, -1, self.inner_dim), scores1, scores2
        else:
            return outer.reshape(B*n_vars*self.patch_num, -1, self.inner_dim)
